    ...
    methods: {
        loadData(){
            axios.get("/api/clients/current").then (e=>{
                ...
                this.creditCard = this.clientCards.filter(c=> c.cardType === "CREDIT");
                this.debitCard = this.clientCards.filter(c=> c.cardType === "DEBIT");
        ...
        createCard(){
          ...
          .then(()=>Swal.fire({
              position: 'center',
              icon: 'success',
              title: 'Card solicitada con exito',
              showConfirmButton: false,
              timer: 1000
            }))
            .catch(Swal.fire('No puedes obtener este tipo de tarjeta.')
            )
